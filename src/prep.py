...
def census_data():
    ...
    df_poverty.columns = ['year','county_code', 'poverty_rate','poverty_rate_white', 'poverty_rate_african_american','poverty_rate_native_american', 'poverty_rate_asian', 'poverty_rate_pacific_islander','poverty_rate_hispanic', 'college_degree']

    df_income = df_income[['year','id2', 'households;_estimate;_mean_income_(dollars)','families;_estimate;_$10,000_to_$14,999']]
    df_income.columns = ['year','county_code', 'household_income','low_income_families']

    df_employment = df_employment[['year','id2', 'unemployment_rate;_estimate;_population_16_years_and_over','employed;_estimate;_disability_status_-_with_any_disability']]
    df_employment.columns = ['year','county_code', 'unemployment_rate','disability_employed']

    #merge dataframes
    census_data = df_income.merge(df_poverty, on=['year','county_code']).merge(df_employment, on=['year','county_code'])
...
def get_all_data():
    ...
    mcd_pivot = mcd.pivot_table(index=['year','county_code'], columns='multiple_cause_of_death_code', values='deaths').reset_index()
