...
def plot_counties(top_counties, col_name, title, legend_title):
    ...
    fig = ff.create_choropleth(fips=top_counties.index.values, values=top_counties[col_name],    binning_endpoints=list(np.linspace(0.0, top_counties[col_name].max(
    ), 11)),     county_outline={'color': 'rgb(255,255,255)', 'width': 0.5}, round_legend_values=False, title=title, legend_title=legend_title)
...
def find_top_counties(mcd, mcd_wide):
    ...
    print(top_counties[['death_ratio', 'county']].sort_values(
        'death_ratio', ascending=False).head())
    print(top_counties[['synthetic_ratio', 'county']].sort_values(
        'synthetic_ratio', ascending=False).head())

    # plot counties
    top_counties = top_counties.sort_values('death_ratio', ascending=False)
    plot_counties(top_counties, 'synthetic_ratio',
                  title="U.S. Counties by Mutliple Cause of Death Involving Synthetic Opioids 2010 to 2016", legend_title='Deaths per 1000 Residents')
    plot_counties(top_counties, 'death_ratio', title="U.S. Counties by Mutliple Cause of Death Involving Drugs 2010 to 2016",
                  legend_title='Deaths per 1000 Residents')


def plot_census(mcd_main):
    """Plots assocations between census data and target feature"""
    predictor_deaths_vs_poverty_rate = mcd_main[['T40.4', 'poverty_rate', 'poverty_rate_white', 'poverty_rate_african_american',
                                                 'poverty_rate_native_american', 'poverty_rate_asian', 'poverty_rate_pacific_islander', 'poverty_rate_hispanic']]

    predictor_deaths_vs_other = mcd_main[['T40.4', 'college_degree',
                                          'low_income_families', 'unemployment_rate', 'disability_employed']]
    ...
    nice_scatter_matrix(predictor_deaths_vs_poverty_rate,
                        'Synthetic Opioid Deaths and Poverty Rate')
    plt.show()
    nice_scatter_matrix(predictor_deaths_vs_other,
                        'Synthetic Opioid Deaths and Other Economic Factors')
    plt.show()
...
def main():
    # Get data from the prep script
    mcd, mcd_main, mcd_wide = get_all_data()

    # Feature engineering for total deaths and death ratios
    ...
    # Use new dataset to visualize statistics by county
    find_top_counties(mcd, mcd_wide)

    # Create scatter matrix of variables of interest to explore relationships
    plot_t40_associations(mcd_wide)
    plot_census(mcd_main)

    # Run an Analysis of variance Hypothesis test to see trends in drugs over years
    deaths_by_MCD = mcd.groupby(by=['year', 'multiple_cause_of_death_code']).sum()[
        'deaths'].reset_index()
    drugs_test = deaths_by_MCD.query('multiple_cause_of_death_code in ["T40.4" ,  "T40.5"]')
    print(ANOVA(drugs_test))

    # Create a dataframe with T40 deaths as the target
    T40 = drop_nulls(mcd_main, ['T40.4'])

    # Iterate through various imputation techniques to determine lowest error
    MSE_dict = determine_impute(T40)

    # Used K-nearest neighbors to impute missing data points
